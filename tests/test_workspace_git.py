...
def test_workspace_git_static_contracts():
    ...
    assert ".toast{pointer-events:none;position:fixed;top:24px;right:24px;left:auto;bottom:auto;transform:translateY(-6px);display:flex;align-items:flex-start" in style
    assert ".toast.show{opacity:1;transform:translateY(0);pointer-events:auto;}" in style
...
def test_git_commit_hook_failure_returns_hook_failed_code(tmp_path, monkeypatch):
    from api.workspace_git import WORKSPACE_GIT_DESTRUCTIVE_ENV, GitWorkspaceError, git_commit, git_stage
