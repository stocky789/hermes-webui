...
def _run_cron_tracked(
    ...
    try:
        ...
        try:
            _with_cron_home(profile_home, lambda: mark_job_run(job_id, False, error_message))
        except Exception:
...
def _git_session(handler, session_id: str):
    ...
    try:
        ...
        # Persist new sessions the first time a git operation touches them.
        # New sessions are only written to disk when the first chat message is
        # sent, but workspace git actions (fetch/pull/push/status) can happen
        # before any message.  Without this save the session vanishes from the
        # backend after a server restart, causing every subsequent git request
        # to return 404 "Session not found".
        session_path = getattr(session, "path", None)
        if session_path is not None and not session_path.exists():
            try:
                session.save(touch_updated_at=False)
            except Exception:
                pass  # best-effort; git op proceeds whether or not save worked
        return session
    except KeyError:
